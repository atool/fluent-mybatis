...
import cn.org.atool.fluent.mybatis.handler.IEnum;
import cn.org.atool.fluent.mybatis.exception.FluentMybatisException;
import cn.org.atool.fluent.mybatis.handler.EnumTypeHandler;
...
import org.apache.ibatis.session.MybatisSqlSessionFactoryBuilder;
...
public class MybatisSqlSessionFactoryBean implements FactoryBean<SqlSessionFactory>, InitializingBean, ApplicationListener<ApplicationEvent> {
    ...
    protected SqlSessionFactory buildSqlSessionFactory() throws Exception {
        ...
        if (hasLength(this.typeEnumsPackage)) {
            Set<Class<?>> classes;
            if (typeEnumsPackage.contains(Constants.STAR) && !typeEnumsPackage.contains(Constants.COMMA)
                && !typeEnumsPackage.contains(Constants.SEMICOLON)) {
                ...
                SimpleAssert.notNull(typeEnumsPackageArray, "not find typeEnumsPackage:" + typeEnumsPackage);
                Stream.of(typeEnumsPackageArray).forEach(typePackage -> {
                    try {
                        ...
                        throw new FluentMybatisException("Cannot scan class in '[" + typePackage + "]' package", e);
                    }
                });
            }
            // 取得类型转换注册器
        ...
        if (globalConfig.isBanner()) {
            System.out.println(StringUtils.getVersionBanner());
        }
